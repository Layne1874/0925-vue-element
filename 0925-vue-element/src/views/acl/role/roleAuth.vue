<template>
	<div>
		<el-card>
			...
			<el-tree style="margin: 40px 0" ref="tree" :data="allPermissions" node-key="id" show-checkbox
...
export default {
	...
	methods: {
	  ...
	  */
		getPermissions(roleId) {
			this.$api.permission.toAssign(roleId).then((result) => {
				...
				/** 
				 *  setCheckedKeys函数的调用需要放入一个数组，数组里面存储的是需要勾选的每一个级及其children的id，
				 * 放入哪个id，哪个就呈现勾选状态，无论在哪一级
				*/
	  ...
	  */
		getCheckedIds(auths, initArr = []) {
			return auths.reduce((pre, item) => {
				// 必须select勾选，且到了四级才需要push，因为只需要收集四级的id
	  ...
	  */
		save() {
			//getCheckedKeys()方法是获得页面树形控件所有选中的id，然后再用join方法切割成字符串
